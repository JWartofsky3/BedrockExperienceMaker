...
import { createPack, listPacks, type ExperiencePack } from '../api'
...
export default function PacksPage() {
  ...
  useEffect(() => { void listPacks().then(setPacks).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Could not load packs.')) }, [])
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    setError(undefined)
    try {
      const pack = await createPack({ displayName: String(form.get('displayName')), creatorName: String(form.get('creatorName')), description: String(form.get('description')) })
  ...
  return <Layout><section className="mx-auto grid max-w-6xl gap-10 px-6 py-12 lg:grid-cols-[2fr_1fr]">
    ...
    <form onSubmit={create} className="h-fit rounded-xl border border-slate-800 bg-slate-900 p-6"><h2 className="text-lg font-semibold">Create a pack</h2>{error && <p className="mt-3 text-sm text-red-300">{error}</p>}
      ...
      <label className="mt-4 block text-sm">Creator name<input required name="creatorName" className="mt-1 w-full rounded border border-slate-700 bg-slate-950 p-2" /></label>
